function [bestmem, bestval, nfeval] = OPTMDE(fname, VTR, D, XVmin, XVmax, ...
    y, NP, itermax,...
    F, CR, strategy, refresh)

    fname = @EvalObjEGO;
% function [bestmem, bestval, nfeval] = srgtsOPTMDE(fname, VTR, D, XVmin, XVmax, ...
%     y, NP, itermax,...
%     F, CR, strategy, refresh)
...
% 	XVmin = [-2 -2];
% 	XVmax = [2 2];
% 	[bestmem,bestval,nfeval] = srgtsOPTMDE("f",VTR,D,XVmin,XVmax);
%
...
%
% Due to the vectorized expressions srgtsOPTMDE executes fairly fast
...
err = [];
if nargin < 1
    error('srgtsOPTMDE 1st argument must be function name');
else
    if exist(func2str(fname)) < 1
        err(1,length(err)+1) = 1;
    end
end
...
if nargin < 12
    ...
    if length(refresh) ~= 1;
        err(1,length(err)+1) = 12;
    end
end

if length(err) > 0
    fprintf(stdout,'error in parameter %d\n', err);
    usage('srgtsOPTMDE (string,scalar,scalar,vector,vector,any,integer,integer,scalar,scalar,integer,integer)');
end
